...
class PhotoCategoryController extends AdminController
{
    ...
    public function actionCrop()
    {
        try {
            $name = $_POST['nameFull'];
            $fullImagePath = Yii::getPathOfAlias('webroot.uploads.images.albums').DIRECTORY_SEPARATOR.$name;
            $quality = 100;
            ...
            $function($resizeImage, Yii::getPathOfAlias('webroot.uploads.images.albums').DIRECTORY_SEPARATOR.$name, $quality);
            $model = new PhotoCategory();
            echo CHtml::image(Yii::app()->baseUrl.'/uploads/images/albums/'.$name);
    ...
    public function actionUpload()
    {
        ...
        $image->saveAs(Yii::getPathOfAlias('webroot.uploads.images.albums').DIRECTORY_SEPARATOR.$name.$type);
        list($resource['width'], $resource['height']) = getimagesize(Yii::getPathOfAlias('webroot.uploads.images.albums').DIRECTORY_SEPARATOR.$name.$type);

        echo CHtml::image(Yii::app()->baseUrl.'/uploads/images/albums/'.$name.$type, '', array('id'=>'crop'));
